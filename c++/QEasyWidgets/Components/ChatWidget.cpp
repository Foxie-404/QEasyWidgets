...
void ChatWidgetBase::_storeAvatar(AvatarDisplay *avatarDisplay, const QString &role) {
    if (avatarDisplay && !avatarDisplay->pixmap().isNull()) {
        m_avatars[role] = *avatarDisplay->pixmap();
    }
}
...
void ChatWidgetBase::setupUI() {
    ...
    connect(m_inputEdit, &QTextEdit::returnPressed, this, &ChatWidgetBase::onSendClicked);
}
