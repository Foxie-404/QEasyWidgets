...
DockTitleBar::DockTitleBar(QDockWidget *parent)
    ...
    m_floatButton->setIcon(IconBase::Window_Stack);
...
void DockTitleBar::mouseDoubleClickEvent(QMouseEvent *event) {
    TitleBarBase::mouseDoubleClickEvent(event);
    
    // Toggle floating state on double click
    if (m_dockWidget) {
        bool isFloating = !m_dockWidget->isFloating();
        m_dockWidget->setFloating(isFloating);
        updateFloatButtonIcon(isFloating);
    }
}
...
void DockTitleBar::updateFloatButtonIcon(bool isFloating) {
    if (m_floatButton) {
        m_floatButton->setIcon(isFloating ? 
